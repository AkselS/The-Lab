...
{
	bool result;
	result = InitialiseShader(device, hwnd, L"texture.vs", L"texture.ps");
	if (!result)
	{
		return false;
	}
...
bool TextureShader::InitialiseShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if (FAILED(result))
	{
		...
		{
			MessageBox(hwnd, "texture.vs", "Missing Shader File", MB_OK);
		}
...
void TextureShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	...
	MessageBox(hwnd, "Error compiling shader.  Check shader-error.txt for message.", "texture.vs", MB_OK);
